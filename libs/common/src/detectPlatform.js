...
exports.detectPlatformFromText = detectPlatformFromText;
/** SSH banner, MOTD, /etc/os-release veya uname çıktısından platform tahmini. */
function detectPlatformFromText(raw) {
    ...
    if (text.includes("freebsd"))
        return "freebsd";
    if (text.includes("darwin") || text.includes("macos"))
        return "macos";
    if (text.includes("windows") || text.includes("microsoft"))
        return "windows";
    if (text.includes("linux") || text.includes("openssh"))
        return "linux";
    return null;
}
function mapOsReleaseId(id) {
    switch (id) {
        case "ubuntu":
            return "ubuntu";
        case "debian":
